...
#define HID_REPORT_ID_MOUSE     0x03   // cypherbox fork: composite mouse
...
// Mouse input:     4 bytes — [buttons][dx][dy][wheel]
...
// Mouse button bitmask (cypherbox fork)
...
public:
    ...
    // ─── Mouse Output (cypherbox fork — Report ID 3) ─────────────────────
    // Relative pointer movement, wheel scroll, and button reports share the
    // same BLE HID connection as the keyboard/consumer reports.
    void mouseMove(int8_t dx, int8_t dy, int8_t wheel = 0);
    void mouseButtons(uint8_t buttons, int8_t dx = 0, int8_t dy = 0, int8_t wheel = 0);
    void mouseClick(uint8_t button = HID_MOUSE_LEFT);
